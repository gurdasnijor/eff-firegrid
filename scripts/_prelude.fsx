// scripts/_prelude.fsx
//
// The generic, behavior-free verification harness shared by every foundation
// proof script. This is the single home for the `uniq` / `check` / `section`
// boilerplate that used to be copy-pasted into tests/Suite.fsx, repl.fsx, and
// each scripts/foundation-*.fsx. Loading it replaces those per-scratchpad
// copies with one harness.
//
...
//   - cleanup registration (run after the proof; skipped on failure when
//     PRESERVE=1 so a counterexample stream can be inspected)
//   - script config (the S2 basin name, overridable with S2_BASIN)
//   - a runner that executes a list of proofs, prints one combined summary,
//     and sets the process exit code
//
// Shared helpers must stay generic. Do not add workflow/object behavior,
// hidden stores, fake clients, or event mappers here.
...
/// Script configuration sourced from the environment, with the same defaults
...
/// Mutable run state. One Context is shared across every proof in a run so the
/// final summary aggregates all sections and checks.
type Context =
    { mutable Passed: int
      mutable Failed: int
...
/// A named proof body. The body receives the shared Context so its checks and
/// cleanups land in the same summary as every other proof.
type Proof =
    { Name: string
...
/// Register cleanup to run after the proof. Cleanups run last-in-first-out, so
/// register them in creation order.
...
let private runCleanups (ctx: Context) =
    async {
        if envFlag "PRESERVE" && ctx.Failed > 0 then
            ...
            // Registered LIFO; List is already newest-first, so this is reverse order.
...
/// Run a list of proofs (optionally filtered by the PROOF env var), print one
/// combined summary, run cleanups, and set the process exit code.
