...
module StreamLog =
    ...
    let appendExpected basin codec streamId (StreamVersion expected) events =
        async {
            ...
            let! result = stream basin streamId |> S2.tryAppendWith opts (encodeRecords codec events)

            match result with
            | Ok ack -> return Ok(toRange ack)
            | Error(S2Errors.SeqNumMismatch actual) ->
                return
                    Error
                        { Expected = StreamVersion expected
                          Actual = StreamVersion actual }
            | Error error -> return failwithf "unexpected append failure: %A" error
        }

    let readFrom basin codec streamId (SeqNum from) =
        async {
            let! records = stream basin streamId |> S2.read (S2.FromSeqNum from) 1000
